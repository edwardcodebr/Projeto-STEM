...
def plotar_trajetoria(v0, theta, g):
    theta_rad = np.radians(theta)
    
    # Calcula o tempo de voo total
    t_total = (2 * v0 * np.sin(theta_rad)) / g
    
    # Cria uma lista de tempos de 0 até t_total com um intervalo pequeno
    t_values = np.linspace(0, t_total, num=500)
    
    # Calcula as posições (x, y) para cada instante de tempo
    x_values = v0 * np.cos(theta_rad) * t_values
    y_values = v0 * np.sin(theta_rad) * t_values - 0.5 * g * t_values**2
...
while True:
    ...
    match escolha:
        ...
        case 2:
            ...
            theta = float(input("Digite o ângulo de lançamento (theta) em graus: "))
            g = float(input("Digite a gravidade (g) em m/s^2: "))

            # Calcular e plotar a trajetória
            distancia, altura_maxima = calcular_trajetoria(v0, theta, g)
            print(f"Distância total: {distancia:.2f} m")
            print(f"Altura máxima: {altura_maxima:.2f} m")
            plotar_trajetoria(v0, theta, g)
        case 3:
...
Bronze:
    ...
    Densidade: ~8300 kg/m³''')
        ...
        case 24:
            # Entrada dos parâmetros do circuito pelo usuário
            R = float(input("Digite o valor da resistência (R) em ohms: "))
            L = float(input("Digite o valor da indutância (L) em henrys: "))
            C = float(input("Digite o valor da capacitância (C) em farads: "))
            V_amplitude = float(input("Digite a amplitude da tensão (V_in): "))
            V_frequency = float(input("Digite a frequência da tensão (V_in) em Hz: "))

            # Definindo a tensão de entrada como uma função senoidal
            V_in = lambda t: V_amplitude * np.sin(2 * np.pi * V_frequency * t)

            # Entrada dos parâmetros de simulação pelo usuário
            t0 = float(input("Digite o tempo inicial da simulação (t0): "))
            t_end = float(input("Digite o tempo final da simulação (t_end): "))
